...
import { buildHealthSnapshot, collectStartupChecks, getPythonBridgeStatus } from './startupChecks.js';
...
describe('collectStartupChecks', () => {
    ...
    it('passes when required env is present', () => {
        ...
        const required = checks.filter((c) =>
            ['MONGO_URI', 'MongoDB connected', 'GITHUB_APP_ID', 'GITHUB_APP_PRIVATE_KEY'].includes(c.label),
        );
...
describe('buildHealthSnapshot', () => {
    it('ok when mongo connected and github + python configured', () => {
        ...
        try {
            ...
            assert.ok(h.python.scriptFound);
        } finally {
            process.env = prev;
        }
    });
...
describe('getPythonBridgeStatus', () => {
    it('finds bundled pythonExploit.py', () => {
        const py = getPythonBridgeStatus();
        assert.equal(py.bin, process.platform === 'win32' ? 'python' : 'python3');
        assert.equal(py.scriptFound, true);
    });
});
