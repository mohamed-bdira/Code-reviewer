...
export function formatEnforcerGithubBody(options: {
    ...
    reasons: string[];
    parseError: string | null;
    /** Raw diff lines inside a ```diff fence; omit or empty to skip section */
    diffFencedBody?: string;
    diffWasTruncated?: boolean;
    /** Number of findings written to DB; null when parse failed / not persisted */
